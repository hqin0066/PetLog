	...
	var body: some View {
		ZStack {
			...
			VStack {
				...
				ZStack {
					TabBarAddButtonView(addButtonsPresented: $addButtonsPresented)
						.offset(y: -UIScreen.main.bounds.width / 16)
					...
					.offset(x: self.addButtonsPresented ? -120 : 0,
									y: self.addButtonsPresented ? -90 : -10)
					...
					.offset(x: self.addButtonsPresented ? -40: 0,
									y: self.addButtonsPresented ? -130 : -10)
...
struct TabBarButtonsView_Previews: PreviewProvider {
	static var previews: some View {
		...
		return TabBarButtonsView(addButtonsPresented: .constant(true), pet: newPet)
